...
def test_single_photo_transient_object_is_not_saved(monkeypatch) -> None:
    monkeypatch.setattr(
        reports,
        "get_ai_service",
    ...
    async def fail_upload(*_args, **_kwargs):
        raise AssertionError("a transient-only report must not be uploaded")
    ...
    assert result.status == "not_saved"
    assert result.report_id is None
    assert "일시적" in (result.report_message or "")


def test_persistent_obstacle_is_saved_as_pending(monkeypatch) -> None:
    monkeypatch.setattr(
        reports,
        "get_ai_service",
        lambda: SimpleNamespace(
            analyze=lambda _contents: hazard_analysis("fixed_obstacle", "high")
        ),
    )
    ...
    assert stored[0].hazard_type == "fixed_obstacle"
    assert "경로 계산에 반영되지 않습니다" in (result.report_message or "")
